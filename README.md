...
A personal knowledge base that runs inside [Claude Code](https://claude.ai/claude-code) or Codex. You talk to it with slash commands. It compiles your notes, conversations, and bookmarks into interlinked markdown — source pages, concept pages, entity pages — so you never re-derive the same insight twice.
...
`setup.sh` registers skill symlinks for both agent surfaces by default:
...
That's it — no database, no mandatory API keys. (`/transcribe` optionally uses Volcengine ASR — see `docs/knowhow/toolchain/volcengine-asr.md`.)

Now open your agent in the `selfOS` directory:

```bash
claude
# or open Codex in this folder
```
...
| `/todo list` | Show current task sections such as Today, Pending, Completed, Abandoned, and Pool. |
