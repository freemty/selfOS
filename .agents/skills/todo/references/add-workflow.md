...
7. **Confirm**:
   ```
   ✅ [T007] 回复邮件 @2026-04-29 #admin → Pool
   ```
...
- **文件不存在**：不应发生（初始化时创建），但如果发生：
  - do.md: 创建标准 frontmatter + 空 `Today` / `Pending` / `Pool`
  - read.md: 创建标准 frontmatter + 空 `Today` / `Completed` / `Abandoned` / `Pool`
