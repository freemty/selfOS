...
**Resolve the wiki root by finding the current repository folder containing `CLAUDE.md` or `AGENTS.md` with the `<!-- llm-wiki -->` marker. If invoked outside the repo, use the installed selfOS path or ask for it.** All paths (`wiki/`, `docs/`) are relative to this root. If CWD is not selfOS, use absolute paths.
